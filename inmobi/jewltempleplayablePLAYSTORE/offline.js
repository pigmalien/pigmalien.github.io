{
	"version": 1550691631,
	"fileList": [
		"data.js",
		"c2runtime.js",
		"jquery-2.1.1.min.js",
		"offlineClient.js",
		"images/sprite2-sheet0.png",
		"images/sprite-sheet0.png",
		"images/sprite-sheet1.png",
		"images/sprite3-sheet0.png",
		"images/sprite4-sheet0.png",
		"images/sprite5-sheet0.png",
		"images/sprite6-sheet0.png",
		"images/sprite7-sheet0.png",
		"images/sprite8-sheet0.png",
		"images/sprite9-sheet0.png",
		"images/sprite9-sheet1.png",
		"images/sprite10-sheet0.png",
		"images/sprite11-sheet0.png",
		"images/sprite12-sheet0.png",
		"images/sprite13-sheet0.png",
		"images/sprite14-sheet0.png",
		"images/sprite15-sheet0.png",
		"images/sprite16-sheet0.png",
		"images/sprite17-sheet0.png",
		"images/sprite18-sheet0.png",
		"images/sprite19-sheet0.png",
		"images/sprite20-sheet0.png",
		"images/sprite21-sheet0.png",
		"images/sprite22-sheet0.png",
		"images/sprite23-sheet0.png",
		"images/sprite24-sheet0.png",
		"images/sprite25-sheet0.png",
		"images/sprite26-sheet0.png",
		"images/sprite27-sheet0.png",
		"images/sprite29-sheet0.png",
		"images/sprite28-sheet0.png",
		"images/sprite30-sheet0.png",
		"images/sprite31-sheet0.png",
		"images/tiledbackground.png",
		"images/sprite32-sheet0.png",
		"images/sprite33-sheet0.png",
		"images/sprite34-sheet0.png",
		"images/sprite35-sheet0.png",
		"media/sound_block_explosion.m4a",
		"media/sound_block_explosion.ogg",
		"media/sound_block_explosion_line.m4a",
		"media/sound_block_explosion_line.ogg",
		"media/sound_stage_clear.m4a",
		"media/sound_stage_clear.ogg",
		"media/sound_block_explosion_bomb.m4a",
		"media/sound_block_explosion_bomb.ogg",
		"media/sound_hammer_swing.m4a",
		"media/sound_hammer_swing.ogg",
		"media/sound_break_slate.m4a",
		"media/sound_break_slate.ogg",
		"media/start.m4a",
		"media/start.ogg",
		"media/sound_block_explosion_color.m4a",
		"media/sound_block_explosion_color.ogg",
		"media/sound_block_movead.m4a",
		"media/sound_block_movead.ogg",
		"media/musicbg.m4a",
		"media/musicbg.ogg",
		"icon-16.png",
		"icon-32.png",
		"icon-114.png",
		"icon-128.png",
		"icon-256.png",
		"loading-logo.png"
	]
}